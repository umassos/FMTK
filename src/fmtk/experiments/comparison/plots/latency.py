...
# =====================================================
# 1) Latency vs Num Tasks — separate subplot per req_rate
# =====================================================
...
for idx, rr in enumerate(all_req_rates):
    ...
    for method, entries in data.items():
        ...
        ys = [e["avg_latency"] for e in sorted(entries, key=lambda x: x["num_tasks"]) if e["req_rate"] == rr]
...
plt.tight_layout()
plt.savefig("LatencyvsNumTask.pdf")
plt.show()

# =====================================================
# 2) Latency vs Request Rate — separate subplot per num_tasks
# =====================================================
...
for idx, nt in enumerate(all_num_tasks):
    ...
    for method, entries in data.items():
        ...
        ys = [e["avg_latency"] for e in sorted(entries, key=lambda x: x["req_rate"]) if e["num_tasks"] == nt]
        ax.plot(xs, ys, marker='o', label=method)
...
plt.tight_layout()
plt.savefig("LatencyvsRequestRate.pdf")
plt.show()
