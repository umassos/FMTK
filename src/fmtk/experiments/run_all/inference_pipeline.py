from fmtk.experiments.run_all.component_loader import (
    get_model_class,
    get_decoder_class,
    get_encoder_class,
    get_adapter_class,
...
class InferencePipeline:
    def __init__(self, task_name, task_info, pipeline):
        ...
        self.dataset_instance_train = dataset_class(
            self.dataset_cfg, self.task_cfg, split="train"
        )
        self.dataset_instance_test = dataset_class(
            self.dataset_cfg, self.task_cfg, split="test"
        )
        self.dataset_instance_val = dataset_class(
            self.dataset_cfg, self.task_cfg, split="val"
        )
        self.dataloader_train = DataLoader(
            self.dataset_instance_train,
            batch_size=self.task_cfg["train_config"]["batch_size"],
            shuffle=self.task_cfg["train_config"]["shuffle"],
        )
        self.dataloader_val = DataLoader(
            self.dataset_instance_val,
    ...
    def run(self):
        print(
            f"Running inference for model: {self.model_type}_{self.model_name} on dataset: {self.dataset_cfg['dataset_type']}"
        )
        backbone_class = get_model_class(self.backbone_cfg["model_type"])
        logger = Logger(device, "log")
        with (
            logger.measure("backbone", device=logger.device)
            if logger
            else nullcontext()
        ):
            fm_instance = backbone_class(
                self.device,
                self.model_name,
                self.backbone_cfg.get("model_config", None),
            )

        P = Pipeline(fm_instance, logger=logger)
        for path in self.pipeline["paths"]:
            if "decoder" in path:
                decoder_class = get_decoder_class(
                    self.task_cfg["task_type"],
                    decoders[path["decoder"]]["decoder_type"],
                )
                with (
                    logger.measure("decoder", device=logger.device)
                    if logger
                    else nullcontext()
                ):
                    if "decoder_config" in decoders[path["decoder"]]:
                        decoder_instance = decoder_class(
                            **decoders[path["decoder"]]["decoder_config"]
                        )
                    else:
                        decoder_instance = decoder_class()
                    ...
                    print(path)
                    P.add_decoder(
                        decoder_instance, load=True, trained=False, path=path["path"]
                    )
            else:
                P.unload_decoder()
            ...
            if "adapter" in path:
                adapter_class = get_adapter_class(
                    adapters[path["adapter"]]["adapter_type"]
                )
                if "adapter_config" in adapters[path["adapter"]]:
                    adapter_instance = adapter_class(
                        **adapters[path["adapter"]]["adapter_config"]
                    )
                else:
                    adapter_instance = adapter_class()
            ...
            P.train(
                ...
                parts_to_train=path["parts_to_train"],
                cfg=self.task_cfg["train_config"],
                path=path["path"],
            )
            print("Training complete")
            y_test, y_pred = P.predict(
                self.dataloader_test, cfg=self.task_cfg["inference_config"]
            )
            ...
            if (
                self.task_cfg["task_type"] == "regression"
                or self.task_cfg["task_type"] == "forecasting"
            ):
                metrics = {
