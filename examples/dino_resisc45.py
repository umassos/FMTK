...
def train_model(
    ...
    linear_decoder = P.add_decoder(
        LinearDecoder(
            device, cfg={"input_dim": EMBED_DIMS[model_id], "output_dim":45}
        ),
        load=True,
...
def run_multiple(
    ...
    for i in epochs:
        ...
        df.to_csv(f"results/dino_resic45_accuracy_epochs_{i}.csv", index=False)
...
if __name__ == "__main__":
    ...
    dataset_cfg = {
        ...
        "dataset_path": "/home/kgudipaty_umass_edu/FMTK/dataset/RESISC45"
    ...
    model_id = "base"
    samples_per_class = [1, 5, 10, 50, 100, 250]
    train_data = RESIC45Dataset(dataset_cfg, task_cfg, split="train")
    test_data = RESIC45Dataset(dataset_cfg, task_cfg, split="test")
