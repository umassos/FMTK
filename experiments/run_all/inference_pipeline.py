...
from fmtk.metrics import get_mae, get_accuracy, get_mIoU, get_token_f1, get_rouge, get_bleu, get_exact_match, get_gsm8k_accuracy
...
class InferencePipeline:
    def __init__(self,task_name,task_info, pipeline, log_file):
        ...
        self.is_llm = task_info.get('task_type') in (
        ...
        # Normalize batch_size to a list to support sweeping over multiple values
        ...
        if self.is_vlm:
            ...
            self._vlm_collate_fn = vlm_collate_fn
        elif self.is_llm:
            self.dataset_instance_test = dataset_class(self.dataset_cfg, self.task_cfg, split='test')
            ...
            try:
                ...
                self.dataset_instance_val = None
    ...
    def run(self):
        ...
        for path in self.pipeline['paths']:
            ...
            if 'adapter' in path:
                ...
                P.add_adapter(adapter_instance, load=True, train=self.train, path=path['path'])
            else:
                P.unload_adapter()
            ...
            for batch_size in self.inference_batch_sizes:
                ...
                if self.is_vlm:
                    dataloader_test = DataLoader(self.dataset_instance_test, batch_size=batch_size,
                                                 shuffle=self.task_cfg['inference_config']['shuffle'],
                                                 collate_fn=self._vlm_collate_fn)
                elif self.is_llm:
                    dataloader_test = DataLoader(self.dataset_instance_test, batch_size=batch_size,
                                                 shuffle=self.task_cfg['inference_config']['shuffle'])
                else:
                ...
                with logger.measure("inference_total", device=logger.device):
                    labels_raw, preds_raw = P.predict(dataloader_test, cfg=self.task_cfg['inference_config'])

                summary = logger.summary()
                if 'train' not in summary:
                ...
                if self.is_vlm:
                    ...
                    metrics = {
                        ...
                        "decoder memory(MB)":      summary.get(f"add_decoder_{path.get('path', '')}", {}).get('gpu peak', None),
                        "decoder load time(ms)":   summary.get(f"add_decoder_{path.get('path', '')}", {}).get('wall time', None),
                        "encoder memory(MB)":      summary.get(f"add_encoder_{path.get('path', '')}", {}).get('gpu peak', None),
                        "encoder load time(ms)":   summary.get(f"add_encoder_{path.get('path', '')}", {}).get('wall time', None),
                        "adapter memory(MB)":      summary.get(f"add_adapter_{path.get('path', '')}", {}).get('gpu peak', None),
                        "adapter load time(ms)":   summary.get(f"add_adapter_{path.get('path', '')}", {}).get('wall time', None),
                    ...
                    if task_type in ('sentiment', 'text_classification', 'reading_comprehension', 'fact_verification'):
                        metric_key, metric_val = 'accuracy', get_accuracy(labels_raw, preds_raw)
                    elif task_type == 'ner':
                        metric_key, metric_val = 'token_f1', get_token_f1(labels_raw, preds_raw)
                    elif task_type == 'qa':
                        metric_key, metric_val = 'exact_match', get_exact_match(labels_raw, preds_raw)
                    elif task_type == 'summarization':
                        rouge = get_rouge(labels_raw, preds_raw)
                        metric_key = 'rouge1'
                        metric_val = rouge['rouge1']
                    elif task_type == 'translation':
                        metric_key, metric_val = 'bleu', get_bleu(labels_raw, preds_raw)
                    elif task_type == 'math_reasoning':
                        metric_key, metric_val = 'gsm8k_accuracy', get_gsm8k_accuracy(labels_raw, preds_raw)
                    ...
                    metrics = {
                        ...
                        "backbone memory(MB)":    summary['backbone']['gpu peak'],
                        "backbone load time(ms)": summary['backbone'].get('wall time', 0),
                        "decoder memory(MB)":     None,
                        "decoder load time(ms)":  None,
                        "encoder memory(MB)":     None,
                        "encoder load time(ms)":  None,
                        "adapter memory(MB)":     None,
                        "adapter load time(ms)":  None,
                        "train time(ms)":         None,
                        "train mem peak(MB)":     None,
                        "train energy(mJ)":       None,
                        "inference time(ms)":     summary['predict']['wall time'],
                        "inference mem peak(MB)": summary['predict']['gpu peak'],
                        "inference energy(mJ)":   summary['predict']['gpu energy'],
                    ...
                    if task_type in ('regression', 'forecasting'):
                        ...
                        num_classes = decoders[path['decoder']]['decoder_config']['cfg']['output_dim']
                        metric_key, metric_val = 'mIoU', get_mIoU(labels_raw, preds_raw, num_classes=num_classes, ignore_index=255)['mIoU']
                    else:
                        metric_key, metric_val = 'accuracy', get_accuracy(labels_raw, preds_raw)
